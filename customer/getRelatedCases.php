...
session_start();
	...
	//$id=$_POST["ID"];
	$qry="Select * from CP_Case where  CaseType = '".$_SESSION["varname"]."'  order by id";
	//echo $qry;
	$run=mysqli_query($con,$qry);
	$i=1;
	$table="";
	$table.="<thead><tr><th>SR.NO</th><th>Case Name</th><th>Date</th><th>Case On</th><th>Case By</th><th>Case Location</th><th>Case Type</th><th>Description</th><th>Status</th><th>Favor Of</th></tr></thead><tbody>";
	while($row=mysqli_fetch_array($run)){
		...
		//$table.="<td><a href='pgSentimate.php?custid=".$rowi["ID"]." &&description=".$row["CaseDescription"]."' target='_self'>Sentimate</a></td>";
